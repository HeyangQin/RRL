...
void ExecutorState::Process(TaggedNode tagged_node, int64 scheduled_usec) {
  ...
  while (!inline_ready.empty()) {
   ...
   //Start TIME
    ...
    //int64_t start = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now().time_since_epoch()).count();
    //std::cout << "executor.cc," << node->name() << "," << "start_time" << "," << start << std::endl;
	...
	//END TIME
	 ...
	 //Tensor::tensor_m.lock();
	 //int64_t end1 = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now().time_since_epoch()).count();
         //std::cout << "executor.cc," << node->name() << "," << "end_time" << "," << end1 << std::endl;
	 ///Tensor::tensor_m.unlock();
    ...
    if (!launched_asynchronously) {
      ...
      //Tensor::tensor_m.lock();
      //int64_t end2 = std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now().time_since_epoch()).count();
      //std::cout << "executor.cc," << node->name() << "," << "end_time" << "," << end2 << std::endl;
      //Tensor::tensor_m.unlock();
